...
- **User Authentication**: OTP-based registration and login system with support for both mobile number and email
...
SMTP_FROM="CFA App <your-email@example.com>"
...
- `POST /api/auth/login` - User login (supports both mobile number and email)

#### Password Management
- `POST /api/auth/forgot-password` - Send OTP for password reset (mobile)
- `POST /api/auth/forgot-password-email` - Send OTP for password reset (email)
- `POST /api/auth/reset-password` - Reset password with OTP (mobile)
- `POST /api/auth/reset-password-email` - Reset password with OTP (email)
...
### Dual Login Support
The API supports login using either:
- **Mobile Number**: Traditional mobile number + password login
- **Email**: Email address + password login
...
#### Mobile-based Reset
1. `POST /api/auth/forgot-password` - Send OTP to mobile
2. `POST /api/auth/reset-password` - Reset with OTP

#### Email-based Reset
1. `POST /api/auth/forgot-password-email` - Send OTP to email
2. `POST /api/auth/reset-password-email` - Reset with OTP

### Registration Flow
1. `POST /api/auth/send-otp` - Send OTP to mobile number
...
- Dual authentication methods (mobile/email)
