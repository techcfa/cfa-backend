...
const connectDB = async () => {
  ...
  try {
      // Prefer the requested seed email
      const seedEmail = 'admin@admin.com';
      let adminExists = await Admin.findOne({ email: seedEmail });
      if (!adminExists) {
        // fallback check by username
        adminExists = await Admin.findOne({ username: 'admin' });
      }
      ...
      if (!adminExists) {
        ...
        await admin.save();
        console.log(`Admin user created successfully: ${seedEmail} / admin123`);
      } else {
