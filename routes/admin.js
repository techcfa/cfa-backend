 ...
 *     description: Creates a new admin. Requires an authenticated admin.
 *     tags: [Admin]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *         description: Internal server error
 */
router.post('/create', adminAuth, validateCreateAdmin, async (req, res) => {
  try {
...
router.get('/dashboard', adminAuth, async (req, res) => {
  try {
    ...
    const recentUsers = await User.find()
      .sort({ createdAt: -1 })
      .limit(5)
      .select('fullName email customerId createdAt');
